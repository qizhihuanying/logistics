		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','outbound_quantity') || $check_field('add','outbound_quantity') || $check_field('set','outbound_quantity')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="出库数量" prop="outbound_quantity">
					<el-input-number id="outbound_quantity" v-model.number="form['outbound_quantity']"
						v-if="user_group === '管理员' || (form['outbound_record_id'] && $check_field('set','outbound_quantity')) || (!form['outbound_record_id'] && $check_field('add','outbound_quantity'))" :disabled="disabledObj['outbound_quantity_isDisabled']"></el-input-number>
					<div v-else-if="$check_field('get','outbound_quantity')">{{form['outbound_quantity']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','delivery_invoice') || $check_field('add','delivery_invoice') || $check_field('set','delivery_invoice')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="出库发票" prop="delivery_invoice">
					<div v-if="disabledObj['delivery_invoice_isDisabled']">
						<div v-if="$check_field('get','delivery_invoice')">
							<el-button type="primary" @click="$download($fullUrl(form['delivery_invoice']),form['delivery_invoice'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
					<div v-else>
						<el-upload v-if="user_group === '管理员' || (form['outbound_record_id'] && $check_field('set','delivery_invoice')) || (!form['outbound_record_id'] && $check_field('add','delivery_invoice'))" class="upload-demo" drag
								   action="" style="max-width: 300px;width: 100%;" :http-request="upload_delivery_invoice" :limit="1" accept="">
							<i class="el-icon-upload"></i>
							<div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
						</el-upload>
						<div v-else-if="$check_field('get','delivery_invoice')">
							<el-button type="primary" @click="$download($fullUrl(form['delivery_invoice']),form['delivery_invoice'])">下载<i
									class="el-icon-download el-icon--right"></i></el-button>
						</div>
					</div>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','freight_vehicle') || $check_field('add','freight_vehicle') || $check_field('set','freight_vehicle')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="货运车辆" prop="freight_vehicle">
					<el-select id="freight_vehicle" v-model="form['freight_vehicle']"
						v-if="user_group === '管理员' || (form['outbound_record_id'] && $check_field('set','freight_vehicle')) || (!form['outbound_record_id'] && $check_field('add','freight_vehicle'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','freight_driver') || $check_field('add','freight_driver') || $check_field('set','freight_driver')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="货运司机" prop="freight_driver">
					<el-select id="freight_driver" v-model="form['freight_driver']"
						v-if="user_group === '管理员' || (form['outbound_record_id'] && $check_field('set','freight_driver')) || (!form['outbound_record_id'] && $check_field('add','freight_driver'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','estimated_time') || $check_field('add','estimated_time') || $check_field('set','estimated_time')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="预计时间" prop="estimated_time">
					<el-date-picker :disabled="disabledObj['estimated_time_isDisabled']" v-if="user_group === '管理员' || (form['outbound_record_id'] && $check_field('set','estimated_time')) || (!form['outbound_record_id'] && $check_field('add','estimated_time'))" id="estimated_time"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','actual_time') || $check_field('add','actual_time') || $check_field('set','actual_time')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="实际时间" prop="actual_time">
					<el-date-picker :disabled="disabledObj['actual_time_isDisabled']" v-if="user_group === '管理员' || (form['outbound_record_id'] && $check_field('set','actual_time')) || (!form['outbound_record_id'] && $check_field('add','actual_time'))" id="actual_time"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','expense_settlement') || $check_field('add','expense_settlement') || $check_field('set','expense_settlement')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="费用结算" prop="expense_settlement">
					<el-input id="expense_settlement" v-model="form['expense_settlement']" placeholder="请输入费用结算"
							  v-if="user_group === '管理员' || (form['outbound_record_id'] && $check_field('set','expense_settlement')) || (!form['outbound_record_id'] && $check_field('add','expense_settlement'))" :disabled="disabledObj['expense_settlement_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
          ...
          "outbound_quantity_isDisabled": false,
					"delivery_invoice_isDisabled": false,
		...
		methods: {
			 ...
			 */
			upload_delivery_invoice(param){
				this.uploadFile(param.file, "delivery_invoice");
			},
			/**
			 * 获取货运车辆列表
			 */
        ...
        if (this.form["delivery_date"].indexOf("-")===-1){
        ...
        if(this.form["delivery_date"]=="0000-00-00"){
          this.form["delivery_date"] = null;
        }
				if(parseInt(this.form["delivery_date"]) > 9999){
					this.form["delivery_date"] = this.$toTime(parseInt(this.form["delivery_date"]),"yyyy-MM-dd")
				}
