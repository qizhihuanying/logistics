<template>
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','employee_gender') || $check_field('add','employee_gender') || $check_field('set','employee_gender')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="员工性别" prop="employee_gender">
					<el-select id="employee_gender" v-model="form_sub['employee_gender']"
						v-if="user_group === '管理员' || (form_sub['ordinary_staff_id'] && $check_field('set','employee_gender')) || (!form_sub['ordinary_staff_id'] && $check_field('add','employee_gender'))">
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["ordinary_staff_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?ordinary_staff_id=" + form_sub["ordinary_staff_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/ordinary_staff/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/ordinary_staff/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/ordinary_staff/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/ordinary_staff/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/ordinary_staff/view','get');
